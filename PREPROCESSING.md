...
- `(lo, hi)`를 매 이미지마다 새로 계산(percentile 등)하면, 같은 raw 픽셀 값이라도 이미지마다 다른 회색
  ...
  확장자)당 한 번만** 고정하고, 이후 모든 이미지에 동일하게 재사용한다.
- **기본값은 고정 `(0, 255)`** (`AnalysisPage._load_current_file`에서 `display_range is None`일 때 직접
  대입). 이전에는 첫 이미지의 1/99 percentile을 자동 계산해 기본값으로 썼으나, 이미지마다 들쭉날쭉한
  기본값 대신 항상 예측 가능한 `0`~`255`로 시작하도록 변경. `image_io.default_display_range()`(percentile
  계산 함수)는 라이브러리 함수로 남아있지만 GUI 기본 흐름에서는 더 이상 호출하지 않음.
- `AnalysisPage.display_range`가 이 고정값을 들고 있으며, `load_image(..., display_range=self.display_range)`로
  매 파일 로드에 전달한다.
- 확장자를 바꾸면(`.raw`↔`.bmp`, 네이티브 값 범위가 보통 크게 다름) `display_range`를 `None`으로 리셋해
  새 확장자에서 다시 `(0, 255)` 기본값으로 시작한다. 반면 raw의 width/height만 바꾸는 것은 같은 바이트를
  다르게 reshape하는 것뿐이라 값 분포(히스토그램) 자체는 안 바뀌므로 `display_range`를 리셋하지 않는다.
...
background_min    = min(background_points)
background_max    = max(background_points)
background_std    = std(background_points)   # population std, ddof=0

profile_min = min(P)    # 제외 구간 없이 라인 전체(신호점 포함) 기준 — 빠른 참고용 보조 지표
profile_max = max(P)
profile_std = std(P)
```

- `background_points`가 비어있으면(예: `a`가 너무 커서 전체 프로파일을 덮음) `background_mean`/`min`/`max`/
  `std` 모두 `NaN` + GUI에 경고 표시. 예외를 던지지 않음.
- `a` 값을 바꾸면 같은 프로파일에 대해 즉시 재계산되어야 함 (라이브 갱신, GUI.md 참고).
- `background_*`(제외 구간 밖)와 `profile_*`(라인 전체, 신호점 포함)는 서로 다른 모집단이라는 점에 주의 —
  `profile_min`은 거의 항상 `signal`과 같은 값이 된다(프로파일의 전역 최저값이 곧 signal이므로).
...
measure(image, line, rect, exclusion_margin) -> {
    "signal", "background_mean", "background_min", "background_max", "background_std",
    "profile_min", "profile_max", "profile_std",
    "noise", "noise_min_max", "rect_min", "rect_mean",
    "area_px", "cnr",
...
3. 결과: `[{filename, signal, background_mean, background_min, background_max, background_std, profile_min, profile_max, profile_std, noise, noise_min_max, rect_min, rect_mean, area_px, cnr, error}, ...]` 리스트.
